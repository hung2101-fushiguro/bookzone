...
public class UserDao implements IUserDAO {
    ...
    public boolean updateUser(User user) throws SQLException {
        ...
        if (hasPassword) {
            sql = "UPDATE Users SET username = ?, email = ?, password = ?, name = ?, avatar_url = ?, provider = ?, provider_id = ?, role = ?, created_at = ?, address = ?, phone = ? WHERE id = ?";
        } else {
            sql = "UPDATE Users SET username = ?, email = ?, name = ?, avatar_url = ?, provider = ?, provider_id = ?, role = ?, created_at = ?, address = ?, phone = ? WHERE id = ?";
        }
        ...
        try (Connection con = DBConnection.getConnection(); PreparedStatement pst = con.prepareStatement(sql)) {
            ...
            pst.setString(i++, user.getAvatarUrl());
            pst.setString(i++, user.getProvider());
            pst.setString(i++, user.getProviderId());
            pst.setString(i++, user.getRole());
            pst.setTimestamp(i++, new Timestamp(user.getCreatedAt().getTime()));
